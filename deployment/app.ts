...
export class ReactAppStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: ReactAppStackProps = {}) {
    ...
    const websiteBucket = new s3.Bucket(this, 'WebsiteBucket', {
      ...
      websiteErrorDocument: 'index.html', // Important for React Router - all routes should serve index.html
      ...
      // Removal policy - change to RETAIN for production
      removalPolicy: envName === 'prod' ? RemovalPolicy.RETAIN : RemovalPolicy.DESTROY,
      autoDeleteObjects: envName !== 'prod', // Only auto-delete in non-prod environments
      
      // Enable versioning for better file management
      versioned: true,
      
      // CORS configuration for API calls (if needed in the future)
      cors: [
        {
